...
from src.utils import extraer_datos, extraer_direccion
...
class ProyectoScraper:
    ...
    def scrapear(self) -> pd.DataFrame:
        ...
        # 2) Extraer dirección (repetir para cada título)
        direccion = extraer_direccion(self.soup)
        direcciones = ([direccion] * len(titulos)) if direccion else [None] * len(titulos)

        # 3) Armar DataFrame inicial
        df = pd.DataFrame({
            "TITULO": titulos,
            "DIRECCION": direcciones
        })

        # 4) Parsear campo TITULO en columnas numéricas y textuales
